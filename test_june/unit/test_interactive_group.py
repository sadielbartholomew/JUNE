...
def test__substract_information_from_group():
    ...
    person3.susceptible = False
    interactive_group = InteractiveGroup(hospital)
    assert len(interactive_group.infectors_ids) == 3
    assert (
        interactive_group.infectors_ids[0][0]
        == person1.health_information.infection.transmission.probability
    )
    assert len(interactive_group.susceptible_ids) == 3
    assert len(interactive_group.susceptible_ids[0]) == 1
    assert len(interactive_group.susceptible_ids[1]) == 0
    assert len(interactive_group.susceptible_ids[2]) == 1
